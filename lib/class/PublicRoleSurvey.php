...
class PublicRoleSurvey extends Survey {
    ...
    public function raphael() {
        ...
        foreach($this->data as $rkey => $rval) {
            ...
            foreach($rval['aspects'] as $akey => $aval) {
                ...
                $js[] =	'st_' . $rkey . '_' . $akey . '.push(t_' . $rkey . '_' . $akey . '_box, t_' . $rkey . '_' . $akey . ');';

                // use raphael hover
                $js[] = 's.hover(function (event) { this.attr({opacity: 0.25, "cursor": "pointer"}); st_' . $rkey . '_' . $akey . '.toFront().show();}, function (event) {this.attr({opacity: 1});st_' . $rkey . '_' . $akey . '.hide();});';
    ...
    /**
     * Create a raphael path for a sector
     */
